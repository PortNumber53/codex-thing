...
```bash
codex resume --remote ws://127.0.0.1:40002 
```
...
- `CODEX_BOOTSTRAP_THREADS` — comma-separated thread IDs that Go resumes when its app-server starts and always includes in Recent Sessions (defaults to ``)
...
Threads use `workspace-write` sandboxing and `approvalPolicy: never`, so the browser cannot grant sandbox escalation requests. The web bridge also rejects interactive app-server requests; unsupported tools fail safely and Codex can continue or explain the limitation.
